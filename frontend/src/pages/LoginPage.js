...
const Brand   = styled.p`
  ...
  font-size: 1rem; margin: 0 0 24px; letter-spacing: 0.05em;
`;
...
const Form    = styled.div`display: grid; gap: 16px;`;
...
const Btn     = styled.button`
  ...
  color: #fff; box-shadow: 0 12px 30px rgba(255,111,177,0.3);
  transition: transform 0.2s;
  &:hover:not(:disabled) { transform: translateY(-2px); }
  &:disabled { opacity: 0.6; cursor: not-allowed; }
`;
const ErrMsg  = styled.p`
  background: rgba(214,51,132,0.08); color: ${({ theme }) => theme.colors.primaryStrong};
  border-radius: 12px; padding: 10px 14px; font-size: 0.88rem; margin: 0;
`;
  ...
  return (
    <Page>
      <Card>
        ...
        <Sub>Sign in to continue booking</Sub>
        ...
        <Switch style={{ marginTop: 20 }}>
          No account? <SLink to="/register">Create one</SLink>
        </Switch>
      </Card>
    </Page>
  );
